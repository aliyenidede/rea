...
_Status: pre-build, learning phase — Archon (`coleam00/Archon`) selected as the harness platform; OpenCode provider integration is in upstream PRs (#1372, #1384). Learning Archon and waiting for OpenCode merge before committing to fork-or-use._
...
- **AI runtime under Archon:** **OpenCode** (`anomalyco/opencode`, formerly `sst/opencode`). Used through Archon's provider abstraction once PR #1372 / #1384 merge. Until then: Claude Code via Claude Agent SDK is the bridge for learning Archon (Boris Cherny verified personal use is allowed). No fork of OpenCode core.
- **Editor:** code-server (Coder), self-hosted on a separate subdomain (`code.<domain>`). Archon Web UI lives at `agent.<domain>` (or similar). Two reverse-proxy entries, same VPS, shared workspace volume.
- **AI provider preference:** OpenRouter primary once OpenCode adapter merges. Anthropic / OpenAI / others remain available through OpenCode's provider system but are not the default.
...
- **Custom Dockerfile** — default Archon Docker image'ında Claude Code yok, README "ships pre-installed" yanıltmış. `Dockerfile.user.example` variant'ı kullanılmalı, ya da host'taki binary mount edilmeli, ya da container içine install edilmeli. Setup zamanı netleşecek.
- **Pi → Gemini erişim ipucu** — Pi multi-model destekliyor. Pi Archon'a girince (Cole'un sözüne göre yaklaşık önümüzdeki hafta), Gemini gibi SDK'sı olmayan modellere Pi üzerinden dolaylı erişim mümkün. Bu ileride değerli.
