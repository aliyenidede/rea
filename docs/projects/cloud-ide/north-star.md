...
- **REA prensiplerinin Archon workflow'a port edilmesi** — Cole'un canlı demosunda gördüğümüz gibi 5-10 dakikalık iş ama hangi prensiplerle başlanacak? A (grilling), B (layered plan) açık adaylar. **Başlangıç malzemesi hazır (doğrulandı 2026-07-01):** Archon ~19 default workflow (`.archon/workflows/defaults/`) + bir **workflow marketplace** ([archon.diy/workflows](https://archon.diy/workflows/), ~9 community workflow, `archon workflow install <slug>`) + `archon-workflow-builder` (workflow üreten workflow) ile geliyor. Sıfırdan yazmak yerine default kopyala / marketplace'ten kur / builder ile scaffold et. Detay: [`deployment-notes.md`](deployment-notes.md).
